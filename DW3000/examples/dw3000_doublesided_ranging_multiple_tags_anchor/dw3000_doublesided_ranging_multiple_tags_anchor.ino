...
void loop()
{
  switch (curr_stage) {
    ...
    case 1:  // Ranging received. Sending response.
      uint32_t rx_data = DW3000.read(0x12, 0x00);
      uint8_t destination_id = (rx_data >> 16) & 0xFF;
      if (device_id == destination_id) {
        uint8_t sender_id = (rx_data >> 8) & 0xFF;
        DW3000.ds_sendFrame(2, device_id, sender_id);
        ...
        Serial.println("[INFO] Destination Id not this device, ignoring.");
        DW3000.clearSystemStatus();
      }
      break;
    ...
    case 3:  // Second response received. Sending information frame.
      ...
      if (device_id == destination_id) {
        ...
        rx = DW3000.readRXTimestamp();
        t_roundB = rx - tx;
        DW3000.ds_sendRTInfo(t_roundB, t_replyB, device_id, sender_id);
        curr_stage = 0;
      } else {
        Serial.println("[INFO] Destination Id not this device, ignoring.");
        DW3000.clearSystemStatus();
